...
#if defined(__MINGW32__) || defined(__CYGWIN__) || defined(__ANDROID__) || defined(__APPLE__) || defined(__OpenBSD__) || defined(__GCC__)
...
static inline unsigned char lzma2_prop_from_dic(u32 dicSize) {
    unsigned i;
    for (i = 0; i <= 40; i++)
        if (dicSize <= LZMA2_DIC_SIZE_FROM_PROP(i)) break;
...
struct rzip_control {
    ...
    char * comment;                // if there is a comment, 64 chars max
...
static inline void print_stuff(const rzip_control * control, int level, unsigned int line, const char * file,
...
static inline void print_err(const rzip_control * control, unsigned int line, const char * file, const char * func,
                             const char * format, ...) {
    va_list ap;
    /* mrzip library callback code removed */
    if (control->msgerr) {
...
#define print_progress(...)                           \
    do {                                              \
        if (SHOW_PROGRESS) {                          \
            ...
            if (!progress_flag) progress_flag = true; \
        }                                             \
    } while (0)
