...
bool init_mutex(rzip_control * control, pthread_mutex_t * mutex) {
    if (unlikely(pthread_mutex_init(mutex, NULL))) fatal("Failed to pthread_mutex_init\n");
    return true;
}

bool unlock_mutex(rzip_control * control, pthread_mutex_t * mutex) {
    if (unlikely(pthread_mutex_unlock(mutex))) fatal("Failed to pthread_mutex_unlock\n");
    return true;
}

bool lock_mutex(rzip_control * control, pthread_mutex_t * mutex) {
    if (unlikely(pthread_mutex_lock(mutex))) fatal("Failed to pthread_mutex_lock\n");
    return true;
}

static bool cond_wait(rzip_control * control, pthread_cond_t * cond, pthread_mutex_t * mutex) {
    if (unlikely(pthread_cond_wait(cond, mutex))) fatal("Failed to pthread_cond_wait\n");
    return true;
}

static bool cond_broadcast(rzip_control * control, pthread_cond_t * cond) {
    if (unlikely(pthread_cond_broadcast(cond))) fatal("Failed to pthread_cond_broadcast\n");
    return true;
}

bool create_pthread(rzip_control * control, pthread_t * thread, pthread_attr_t * attr, void * (*start_routine)(void *),
                    void * arg) {
    if (unlikely(pthread_create(thread, attr, start_routine, arg))) fatal("Failed to pthread_create\n");
    return true;
}

bool detach_pthread(rzip_control * control, pthread_t * thread) {
    if (unlikely(pthread_detach(*thread))) fatal("Failed to pthread_detach\n");
    return true;
}

bool join_pthread(rzip_control * control, pthread_t th, void ** thread_return) {
    if (pthread_join(th, thread_return)) fatal("Failed to pthread_join\n");
    return true;
}
...
static void bzip3_compress(rzip_control * control, uchar * c_buf, i64 * c_len, i64 s_len, int ct) {
    ...
    *c_len = bz3_encode_block(state, c_buf, s_len);
    if (bz3_last_error(state) != BZ3_OK) {
        print_err("Failed to compress with bz3: %s\n", bz3_strerror(state));
        return;
    }
...
static void bzip3_decompress(rzip_control * control, uchar * s_buf, i64 * s_len, i64 c_len, int ct) {
    ...
    *s_len = bz3_decode_block(state, s_buf, c_len, *s_len);
    if (bz3_last_error(state) != BZ3_OK) {
        print_err("Failed to decompress with bz3: %s\n", bz3_strerror(state));
        return;
    }
    unlock_state(control, state);
}
...
static int zpaq_compress_buf(rzip_control * control, struct compress_thread * cthread, int current_thread) {
    ...
    if (LZ4_TEST) {
        if (!(compressibility = lz4_compresses(control, cthread->s_buf, cthread->s_len))) return 0;
    ...
    if (zpaq_redundancy > 192) zpaq_type = 1;         /* text data */

    sprintf(method, "%d%d,%d,%d", control->zpaq_level, control->zpaq_bs, zpaq_redundancy, zpaq_type);

    print_verbose(
        "Starting zpaq backend compression thread %d...\nZPAQ: Method selected: %s: level=%d, bs=%d, redundancy=%d, "
        "type=%s\n",
        current_thread, method, control->zpaq_level, control->zpaq_bs, zpaq_redundancy,
        (zpaq_type == 0 ? "binary/random" : "text"));

    zpaq_compress(c_buf, &c_len, cthread->s_buf, cthread->s_len, &method[0], control->msgout,
                  SHOW_PROGRESS ? true : false, current_thread);
...
static int lz4_compress_buf(rzip_control * control, struct compress_thread * cthread) {
    ...
    if (control->compression_level <= 2) {
        ...
        if ((dlen = LZ4_compress_HC((const char *)cthread->s_buf, (char *)c_buf, in_len, dlen,
...
static int zpaq_decompress_buf(rzip_control * control, struct uncomp_thread * ucthread, int current_thread) {
    ...
    dlen = 0;
    zpaq_decompress(ucthread->s_buf, &dlen, c_buf, ucthread->c_len, control->msgout, SHOW_PROGRESS ? true : false,
                    current_thread);
...
ssize_t put_fdout(rzip_control * control, void * offset_buf, ssize_t ret) {
    if (!TMP_OUTBUF) return write(control->fd_out, offset_buf, (size_t)ret);
    ...
    control->out_ofs += ret;
    if (likely(control->out_ofs > control->out_len)) control->out_len = control->out_ofs;
    return ret;
}
...
ssize_t read_1g(rzip_control * control, int fd, void * buf, i64 len) {
    ...
    if (TMP_INBUF && fd == control->fd_in) {
        ...
        if (unlikely(control->in_ofs + len > control->in_maxlen)) {
            ...
            if (dump_stdin(control)) fatal("Inadequate ram to %compress from STDIN and unable to create in tmpfile");
            goto read_fd;
        }
...
/* write a byte */
static inline int write_u8(rzip_control * control, uchar v) { return write_buf(control, &v, 1); }
...
static inline int read_u8(rzip_control * control, int f, uchar * v) { return read_buf(control, f, v, 1); }
    ...
    if (TMP_INBUF) {
        if (spos > control->in_len) {
            ...
            if (control->in_ofs + len > control->in_maxlen) {
                if (unlikely(dump_stdin(control))) return -1;
                goto fd_seek;
            } else {
                if (unlikely(!read_fdin(control, len))) return -1;
            }
        }
        control->in_ofs = spos;
        if (unlikely(spos < 0)) {
...
static i64 get_seek(rzip_control * control, int fd) {
    ...
    if (TMP_OUTBUF) return control->out_relofs + control->out_ofs;
    ret = lseek(fd, 0, SEEK_CUR);
    if (unlikely(ret == -1)) fatal("Failed to lseek in get_seek\n");
    return ret;
}
...
i64 get_readseek(rzip_control * control, int fd) {
    ...
    if (TMP_INBUF) return control->in_ofs;
    ret = lseek(fd, 0, SEEK_CUR);
    if (unlikely(ret == -1)) fatal("Failed to lseek in get_seek\n");
    return ret;
}
...
bool prepare_streamout_threads(rzip_control * control) {
    ...
    if (control->threads > 1) ++control->threads;
    if (NO_COMPRESS) control->threads = 1;
    threads = control->pthreads = calloc(sizeof(pthread_t), control->threads);
    if (unlikely(!threads)) fatal("Unable to calloc threads in prepare_streamout_threads\n");
...
bool close_streamout_threads(rzip_control * control) {
    ...
    for (i = 0; i < control->threads; i++) {
        ...
        if (++close_thread == control->threads) close_thread = 0;
    }
    dealloc(cthreads);
    dealloc(control->pthreads);
    if (states != NULL) {
        for (i = 0; i < control->threads; i++) bz3_free(states[i]);
        free(states);
        free(statequeue);
    }
    return true;
...
void * open_stream_out(rzip_control * control, int f, unsigned int n, i64 chunk_limit, char cbytes) {
    ...
    if (unlikely(!sinfo)) return NULL;
    if (chunk_limit < control->page_size) chunk_limit = control->page_size;
    ...
    if (save_threads == 0) {
        ...
        int thread_limit =
            control->threads >= PROCESSORS / 2 ? (control->threads / 2) : control->threads;  // control thread loop
        if (ZPAQ_COMPRESS) {
        ...
        retry_zpaq:
            ...
            if (control->zpaq_bs != save_bs) print_verbose("ZPAQ Block Size reduced to %'d\n", control->zpaq_bs);
        ...
        retry_bzip3:
            ...
            if (control->bzip3_bs != save_bs) print_verbose("BZIP3 Block Size reduced to %'d\n", control->bzip3_bs);
        }

        if (control->threads != save_threads) print_verbose("Threads reduced to %'d\n", control->threads);
         ...
         */
        if (control->st_size > 0 && control->st_size < limit) limit = MAX(control->st_size, STREAM_BUFSIZE);
...
void * open_stream_in(rzip_control * control, int f, int n, char chunk_bytes) {
    ...
    if (unlikely(!threads)) return NULL;
    ...
    for (i = 0; i < n; i++) {
        ...
        if (unlikely(ENCRYPT && read_buf(control, f, enc_head, SALT_LEN))) goto failed;
    again:
        if (unlikely(read_u8(control, f, &c))) goto failed;
        ...
        if (control->major_version == 0) {
            ...
            if (unlikely(read_val(control, f, &v1, read_len))) goto failed;
            if (unlikely(read_val(control, f, &v2, read_len))) goto failed;
            if (unlikely(read_val(control, f, &sinfo->s[i].last_head, read_len))) goto failed;
        ...
        if (unlikely(c != CTYPE_NONE)) {
            ...
            if (ENCRYPT) print_err("Wrong password?\n");
            goto failed;
        }
        if (unlikely(v1)) {
...
static bool rewrite_encrypted(rzip_control * control, struct stream_info * sinfo, i64 ofs) {
    ...
    if (unlikely(!head)) fatal("Failed to malloc head in rewrite_encrypted\n");
    buf = head + SALT_LEN;
    gcry_create_nonce(head, SALT_LEN);
    if (unlikely(seekto(control, sinfo, ofs - SALT_LEN))) fatal("Failed to seekto buf ofs in rewrite_encrypted\n");
    if (unlikely(write_buf(control, head, SALT_LEN))) fatal("Failed to write_buf head in rewrite_encrypted\n");
    if (unlikely(read_buf(control, sinfo->fd, buf, 25))) fatal("Failed to read_buf buf in rewrite_encrypted\n");

    if (unlikely(!lrz_encrypt(control, buf, 25, head))) goto error;

    if (unlikely(seekto(control, sinfo, ofs))) fatal("Failed to seek back to ofs in rewrite_encrypted\n");
    if (unlikely(write_buf(control, buf, 25))) fatal("Failed to write_buf encrypted buf in rewrite_encrypted\n");
    dealloc(head);
...
retry:
    ...
    if (!ret) {
        if (ENCRYPT) {
             ...
             * data */
            if (padded_len < *control->enc_keylen) padded_len = *control->enc_keylen;
            cti->s_buf = realloc(cti->s_buf, padded_len);
            if (unlikely(!cti->s_buf)) fatal("Failed to realloc s_buf in compthread\n");
    ...
    if (!waited) {
        ...
        while (output_thread != current_thread) cond_wait(control, &output_cond, &output_lock);
    ...
    if (!ctis->chunks++) {
        ...
        if (TMP_OUTBUF) {
            ...
            if (!control->magic_written) write_magic(control);
        ...
        if (!ENCRYPT) write_val(control, ctis->size, ctis->chunk_bytes);
    ...
    print_maxverbose("Compthread %'d seeking to %'" PRId64 " to store length %'d\n", current_thread,
                     ctis->s[cti->streamno].last_head, write_len);
    ...
    if (ENCRYPT) rewrite_encrypted(control, ctis, ctis->s[cti->streamno].last_head - 17);
    ...
    if (unlikely(write_u8(control, cti->c_type) || write_val(control, cti->c_len, write_len) ||
                 write_val(control, cti->s_len, write_len) || write_val(control, 0, write_len))) {
    ...
    lock_mutex(control, &output_lock);
    if (++output_thread == control->threads) output_thread = 0;
...
static void clear_buffer(rzip_control * control, struct stream_info * sinfo, int streamno, int newbuf) {
    ...
    print_maxverbose("Starting thread %'d to compress %'" PRId64 " bytes from stream %'d\n", current_thread,
                     cthreads[current_thread].s_len, streamno);
    ...
    if (++current_thread == control->threads) current_thread = 0;
}
...
retry:
     ...
     * parallel */
    if (unlikely(ret)) {
        if (unlikely(waited)) fatal("Failed to decompress in ucompthread\n");
        print_maxverbose(
...
static int fill_buffer(rzip_control * control, struct stream_info * sinfo, struct stream * s, int streamno) {
    ...
    if (s->eos) goto out;
fill_another:
    if (unlikely(ucthreads[s->uthread_no].busy)) fatal("Trying to start a busy thread, this shouldn't happen!\n");

    if (unlikely(read_seekto(control, sinfo, s->last_head))) return -1;

    if (ENCRYPT) {
        if (unlikely(read_buf(control, sinfo->fd, enc_head, SALT_LEN))) return -1;
        sinfo->total_read += SALT_LEN;
    }

    if (unlikely(read_u8(control, sinfo->fd, &c_type))) return -1;
    ...
    if (control->major_version == 0) {
        ...
        if (ENCRYPT)
            ...
            read_len = sinfo->chunk_bytes;
        if (unlikely(read_val(control, sinfo->fd, &c_len, read_len))) return -1;
        if (unlikely(read_val(control, sinfo->fd, &u_len, read_len))) return -1;
        if (unlikely(read_val(control, sinfo->fd, &last_head, read_len))) return -1;
        header_length = 1 + (read_len * 3);
    }
    sinfo->total_read += header_length;
    ...
    if (ENCRYPT) {
        // pass decrypt flag
        if (unlikely(!decrypt_header(control, enc_head, &c_type, &c_len, &u_len, &last_head, LRZ_DECRYPT))) return -1;
        if (unlikely(read_buf(control, sinfo->fd, blocksalt, SALT_LEN))) return -1;
        sinfo->total_read += SALT_LEN;
    }
    c_len = le64toh(c_len);
    u_len = le64toh(u_len);
    ...
    if (ENCRYPT) {
        if (unlikely(c_len < 1 || u_len < 1 || last_head < 0 || (last_head && (last_head <= s->last_head))))
            fatal("Invalid data compressed len %'" PRId64 " uncompressed %'" PRId64 " last_head %'" PRId64
                  " chunk size %'" PRId64 "\n",
    ...
    if (unlikely(!s_buf)) fatal("Unable to malloc buffer of size %'" PRId64 " in fill_buffer\n", u_len);
    ...
    if (++s->uthread_no == s->base_thread + s->total_threads) s->uthread_no = s->base_thread;
skip_empty:
    ...
    if (!last_head)
        s->eos = 1;
    else if (s->uthread_no != s->unext_thread && !ucthreads[s->uthread_no].busy && sinfo->ram_alloced < control->maxram)
        goto fill_another;
out:
    lock_mutex(control, &output_lock);
    ...
    thr_return = NULL;
    if (unlikely(!join_pthread(control, threads[s->unext_thread], &thr_return) || !!thr_return)) return -1;
    ...
    if (++s->unext_thread == s->base_thread + s->total_threads) s->unext_thread = s->base_thread;
...
void write_stream(rzip_control * control, void * ss, int streamno, uchar * p, i64 len) {
    ...
    while (len) {
        ...
        if (sinfo->s[streamno].buflen == sinfo->bufsize) flush_buffer(control, sinfo, streamno);
    }
}
...
i64 read_stream(rzip_control * control, void * ss, int streamno, uchar * p, i64 len) {
    ...
    while (len) {
        ...
        if (n > 0) {
            if (unlikely(!s->buf)) fatal("Stream ran out prematurely, likely corrupt archive\n");
        ...
        if (len && s->bufp == s->buflen) {
            if (unlikely(fill_buffer(control, sinfo, s, streamno))) return -1;
            if (s->bufp == s->buflen) break;
        }
    }
...
int close_stream_out(rzip_control * control, void * ss) {
    ...
    for (i = 0; i < sinfo->num_streams; i++) clear_buffer(control, sinfo, i, 0);
    ...
    if (ENCRYPT) {
        ...
        for (i = 0; i < sinfo->num_streams; i++) rewrite_encrypted(control, sinfo, sinfo->s[i].last_headofs);
    }
...
static void add_to_rulist(rzip_control * control, struct stream_info * sinfo) {
    ...
    if (unlikely(!node)) fatal("Failed to calloc struct node in add_rulist\n");
    node->sinfo = sinfo;
...
int close_stream_in(rzip_control * control, void * ss) {
    ...
    print_maxverbose("Closing stream at %'" PRId64 ", want to seek to %'" PRId64 "\n",
                     get_readseek(control, control->fd_in), sinfo->initial_pos + sinfo->total_read);
    if (unlikely(read_seekto(control, sinfo, sinfo->total_read))) return -1;

    for (i = 0; i < sinfo->num_streams; i++) dealloc(sinfo->s[i].buf);
...
static int lz4_compresses(rzip_control * control, uchar * s_buf, i64 s_len) {
    ...
    c_buf = malloc(d_len);
    if (unlikely(!c_buf)) fatal("Unable to allocate c_buf in lz4_compresses\n");
    ...
    while (test_len > 0) {
        ...
        if (lz4_ret > 0) {
            ...
            if (lz4_ret < in_len * ((double)control->threshold / 100)) break;
        }
    ...
    print_maxverbose("lz4 testing %s for chunk %'" PRId32 ". Compressed size = %5.2F%% of test size %'d, %'d Passes\n",
                     (return_value > 0 ? "OK" : "FAILED"), s_len, pct, buftest_size, workcounter);
