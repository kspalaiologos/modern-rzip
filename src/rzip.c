	...
	if (st->chunk_size > cksum_limit) {
		...
		while (42) {
			...
			if (cksum_len < control->page_size)
				fatal("Failed to malloc any ram for checksum ckbuf\n");
		}
...
static void add_to_sslist(rzip_control *control, struct rzip_state *st)
{
	...
	if (unlikely(!node))
		fatal("Failed to calloc struct node in add_to_sslist\n");
	node->data = st->ss;
	node->prev = st->sslist;
	st->head = node;
}
