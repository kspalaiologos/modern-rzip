...
static bool fwrite_stdout(rzip_control * control, void * buf, i64 len) {
    ...
    while (len > 0) {
        ...
        if (unlikely(ret != nmemb)) fatal("Failed to fwrite %'" PRId32 " bytes in fwrite_stdout\n", nmemb);
        len -= ret;
        offset_buf += ret;
        total += ret;
    }
...
bool write_fdout(rzip_control * control, void * buf, i64 len) {
    ...
    while (len > 0) {
        ...
        if (unlikely(ret != nmemb)) fatal("Failed to write %'" PRId32 " bytes to fd_out in write_fdout\n", nmemb);
        len -= ret;
        offset_buf += ret;
    }
    return true;
...
bool write_fdin(rzip_control * control) {
    ...
    while (len > 0) {
        ...
        if (unlikely(ret <= 0)) fatal("Failed to write to fd_in in write_fdin\n");
        len -= ret;
        offset_buf += ret;
    }
    return true;
