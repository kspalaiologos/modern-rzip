...
void register_outputfile(rzip_control * control, FILE * f) { control->outputfile = f; }

void unlink_files(rzip_control * control) {
    /* Delete temporary files generated for testing or faking stdio */
    if (control->util_outfile && control->delete_outfile) unlink(control->util_outfile);

    if (control->util_infile && control->delete_infile) unlink(control->util_infile);
}
...
void setup_overhead(rzip_control * control) {
    ...
    if (LZMA_COMPRESS) {
        control->overhead = (1 << control->compression_level) * ONE_MB;
    } else if (ZPAQ_COMPRESS) {
        control->zpaq_level = (control->compression_level < 4 ? 3 : (control->compression_level < 8 ? 4 : 5));
...
bool read_config(rzip_control * control) {
    ...
    if (fp) fprintf(control->msgout, "Using configuration file ./mrzip.conf\n");
    if (fp == NULL) {
        HOME = getenv("HOME");
        if (HOME) {
            snprintf(homeconf, sizeof(homeconf), "%s/.mrzip/mrzip.conf", HOME);
            fp = fopen(homeconf, "r");
            if (fp) fprintf(control->msgout, "Using configuration file %s\n", homeconf);
        }
    }
    if (fp == NULL) {
        fp = fopen("/etc/mrzip/mrzip.conf", "r");
        if (fp) fprintf(control->msgout, "Using configuration file /etc/mrzip/mrzip.conf\n");
    }
    ...
    while ((fgets(line, 255, fp)) != NULL) {
        ...
        if (isparameter(parameter, "window")) {
            ...
            if (isparameter(parametervalue, "yes")) control->flags |= FLAG_UNLIMITED;
            ...
            /* default is yes */
            if (isparameter(parametervalue, "no")) control->flags &= ~FLAG_THRESHOLD;
            ...
            if (isparameter(parametervalue, "yes") || (control->hash_code > 0 && control->hash_code <= MAXHASH))
            ...
            if (!control->outdir) fatal("Fatal Memory Error in read_config\n");
            strcpy(control->outdir, parametervalue);
            if (strcmp(parametervalue + strlen(parametervalue) - 1, "/")) strcat(control->outdir, "/");
        } else if (isparameter(parameter, "verbosity")) {
            if (control->flags & FLAG_VERBOSE) fatal("CONF.FILE error. Verbosity already defined.\n");
            if (isparameter(parametervalue, "yes"))
            ...
            /* Yes by default */
            if (isparameter(parametervalue, "NO")) control->flags &= ~FLAG_SHOW_PROGRESS;
            ...
            if (isparameter(parametervalue, "yes")) control->flags |= FLAG_KEEP_BROKEN;
        } else if (iscaseparameter(parameter, "DELETEFILES")) {
            /* delete files must be case sensitive */
            if (iscaseparameter(parametervalue, "YES")) control->flags &= ~FLAG_KEEP_FILES;
        } else if (iscaseparameter(parameter, "REPLACEFILE")) {
            /* replace mrzip file must be case sensitive */
            if (iscaseparameter(parametervalue, "YES")) control->flags |= FLAG_FORCE_REPLACE;
        } else if (isparameter(parameter, "tmpdir")) {
            control->tmpdir = realloc(NULL, strlen(parametervalue) + 2);
            if (!control->tmpdir) fatal("Fatal Memory Error in read_config\n");
            strcpy(control->tmpdir, parametervalue);
            if (strcmp(parametervalue + strlen(parametervalue) - 1, "/")) strcat(control->tmpdir, "/");
        } else if (isparameter(parameter, "encrypt")) {
            control->enc_code = atoi(parametervalue);
            if (isparameter(parametervalue, "YES") || (control->enc_code > 0 && control->enc_code <= MAXENC))
    ...
    if (unlikely(fclose(fp))) fatal("Failed to fclose fp in read_config\n");

    /*	fprintf(stderr, "\nWindow = %'d \
            \nCompression Level = %'d \
            \nThreshold = %1.2f \
            \nOutput Directory = %s \
            \nFlags = %'d\n", control->window,control->compression_level, control->threshold, control->outdir,
       control->flags);
    */
    return true;
}
...
static void lrz_keygen(const rzip_control * control, const uchar * salt, uchar * key, uchar * iv, int keylen,
                       int ivlen) {
    uchar * buf;
...
bool lrz_crypt(const rzip_control * control, uchar * buf, i64 len, const uchar * salt, int encrypt) {
     ...
     */
    gcry_error = gcry_cipher_open(&gcry_aes_cbc_handle, *control->enc_gcode, GCRY_CIPHER_MODE_CBC,
                                  GCRY_CIPHER_SECURE | GCRY_CIPHER_CBC_CTS);
    if (unlikely(gcry_error)) fatal("Unable to set AES CBC handle in lrz_crypt: %'d\n", gcry_error);
    gcry_error = gcry_cipher_setkey(gcry_aes_cbc_handle, key, *control->enc_keylen);
    if (unlikely(gcry_error)) fatal("Failed to set AES CBC key in lrz_crypt: %'d\n", gcry_error);
    gcry_error = gcry_cipher_setiv(gcry_aes_cbc_handle, iv, *control->enc_ivlen);
    if (unlikely(gcry_error)) fatal("Failed to set AES CBC iv in lrz_crypt: %'d\n", gcry_error);
    ...
    if (encrypt == LRZ_ENCRYPT) {
        ...
        if (unlikely(gcry_error)) fatal("Failed to encrypt AES CBC data in lrz_crypt: %'d\n", gcry_error);
        ...
        if (unlikely(gcry_error)) fatal("Failed to decrypt AES CBC data in lrz_crypt: %'d\n", gcry_error);
    }
...
void lrz_stretch(rzip_control * control) {
    ...
    error = gcry_kdf_derive(control->salt_pass, control->salt_pass_len, GCRY_KDF_SCRYPT, costfactor, control->salt,
                            SALT_LEN, parallelization, HASH_LEN, control->hash);

    if (unlikely(error)) fatal("Unable to hash password. Not enough memory? Error: %'d\n");
}
...
bool decrypt_header(rzip_control * control, uchar * head, uchar * c_type, i64 * c_len, i64 * u_len, i64 * last_head,
    ...
    if (unlikely(!lrz_decrypt(control, buf, 25, head, dec_or_validate))) return false;
